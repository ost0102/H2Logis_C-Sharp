$(function () {
    ...
    $("._btn_write").on('click', function (e) {
        location.href = "/Admin/RecruitmentWrite";
    });
...
function goView(pageID) {
    location.href = "/Admin/RecruitmentWrite?id=" + pageID;
}
...
function _fnSearchData(pageIndex) {
    ...
    $.ajax({
        type: "POST",
        url: "/Admin/Recruitment_CallAjax",
        async: false,
        dataType: "json",
        data: callObj,
    ...
    if (rtnVal == undefined) {
        ...
        $(rtnVal).each(function (i) {
            ...
            if (rtnVal[i].RECRUITMENT_YN == "y") {
                ...
                //}
                innerText += "	<td class='ta_left'><a href='javascript:void(0);' onclick='goView(" + rtnVal[i].RECRUITMENT_ID + ")' class='_btn_edit' data-val='" + rtnVal[i].RECRUITMENT_ID + "'>" + rtnVal[i].TITLE + "</a></td> ";
